...
/**
 * For bullet 1
 *   d. Create alphabet array, for all symbolic variables. Make it in Greek. ;)
 *   Create class MyVariable(empty). ?
 *   Create a global "Variables" array.
 *   Modify MyBool, MyString, MyInt to extend MyVariable. ?
 *   Create private method, used by constructor, which adds the variable to the global "variables" list.
 *   Modify constructors of these variables to accept as input the variable array.
 *   For each variable, in the constructor, pass the global Variables array.
 */

class Alphabet{
	public static String alphabet = "ΑΒΓΔΕΖΗΘΙΚΟΜΝΞΠΡΣΤΥΦΧΨΩαβγδεζηθικλμνξπρστφχψω";
}

/**
 * Class variable.
 *   Holds type, name, and symbolic value of new variable.
 *   Holds a method for generating new names, using the alphabet as input.
 */
class Variable {
	...
	/**
	 * Cases:
	 * First element in variables
	 * previous element has final letter.
	 *   previous is caps.
	 *   		  is lowercase.
	 *  previous element has
	 */
	public void generateName(String alphabet, ArrayList<Variable> variables) {
